...
**Nike Studio Fleece image loading regression after fetchpriority change** — commit `3956458` ("Preload stencils and patches without needing a click, via fetchpriority") replaced click-gated deferred loading of the design/patch grid with always-eager `fetchpriority="low"` loading of all 36 stencils + 8 patches simultaneously from page load, on `pickup_join_nike_studio_fleece.html`. User report after deploy: "hoodies and patches now load in slow" — a regression, since patches were loading fine before this specific change. Note the *original* complaint this change was meant to fix was scoped to the small placement-row thumbnail (the little image shown in the picker row after a selection is made), not the main grid — so the fix's scope was broader than the actual problem.
FIX (commit `51267a5`): straight `git revert 3956458` — clean, no later commits touched the file. Restores the prior verified-working state from `95fdea2`: `preloadPool()` fires on hoodie selection for just that garment's darkness (2 pools, ~15-18 images, not all 36), so a mode click still isn't required to start the fetch — and patches go back to native `loading="lazy"`, which was never implicated. Root cause was not formally isolated (local-runserver-vs-gunicorn was never tested separately) since the revert restores a state already confirmed working live, and the event is imminent — not worth the extra diagnostic time. If it regresses again, look at real network contention from ~44 simultaneous requests before reaching for `fetchpriority` again.
STATUS: RESOLVED — verify once more in a real browser walkthrough before the event as a final sanity check.
