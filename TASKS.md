...
- [x] RESOLVED in PHASE 25f: the client's actual sponsor source turned out to be a different folder (`Kotn Sponsors From Naya/`, matching a shared Drive link) than the one referenced here — see PHASE 25f for the 3 sponsors added from it (Rosie's Burgers, NSS Sports, SLC), bringing the total to 9.
...
## PHASE 26 — Nike Studio Fleece customizer (Nike x JD Sports pop-up, live event, DEPLOYED)

- [x] Full two-theme (`nike-studio-fleece-white` / `nike-studio-fleece-noir`) hoodie customizer, built as a hardcoded slug branch in `customer/views.py` following the Kotn Cup precedent exactly — no schema changes. Flow: hoodie + size → airbrush design (optional) → patches (optional, max 3) → review → phone → real queue/confirmation with SMS.
- [x] `NIKE_STUDIO_FLEECE_SLUGS`, `NIKE_HOODIES`, `NIKE_SIZES`, `NIKE_DESIGNS_LIGHT/DARK_LARGE/SMALL`, `NIKE_PATCHES` (8), `NIKE_MAX_PATCHES = 3`, `NIKE_PATCH_PLACEMENTS` (4 arm/chest keys, only chest spots are exclusive — patches can share an arm), `NIKE_DESIGN_PLACEMENT_LARGE`/`NIKE_DESIGN_PLACEMENTS_SMALL`, `NIKE_ACCENT_A/B`, `NIKE_THEMES` — all in `customer/views.py`.
- [x] `PickupJoinView._post_nike_studio_fleece()` — airbrush and patches are both fully optional (`canContinue()` only requires hoodie + size); server revalidates every field, never trusts client. Sequential per-business order numbers via `select_for_update()`, same scheme as Kotn tags.
- [x] New templates: `pickup_join_nike_studio_fleece.html` (single-page build flow), `pickup_confirmation_nike_studio_fleece.html` (waiting/ready screens), `_nike_build_card.html` (shared "YOUR BUILD" partial).
- [x] `--ink-outline` CSS technique (`filter: drop-shadow`) used for a "no-box" theme-aware outline on garment/design art, replacing solid card-tint backgrounds — patches render with `filter: none` on the white theme per later feedback (shadows removed from patches everywhere: grid, placement row, review, confirmation).
- [x] Dashboard: bespoke `.nike-card` layout in `dashboard/templates/dashboard/queue.html` (thumb + collapsible details, click-to-expand capped at 2 concurrent cards with oldest-eviction) for both `pickup_waiting` and `pickup_ready` loops, plus JS `renderNikeEntry()` dispatched from `renderPickupEntry()`. Expand state persists across the 5s poll re-render via a module-level `nikeExpandedIds` array (fixed a bug where expand silently collapsed on every poll tick).
- [x] Went **white theme only** for the live event per explicit decision ("we are going with that one not the noir") — noir code paths (constants, template branches, static assets) left intact/untouched in case of a revert, just not the active choice. Nike/JD corner brackets and center tick were removed then explicitly restored per follow-up feedback — currently visible in both build and confirmation screens.
- [x] Local dev DB: two `Business` rows (`nike-studio-fleece-white`, `nike-studio-fleece-noir`) plus a shared staff login `+14374402233` ("Nike Staff") for both. Production DB: matching rows created directly via `railway connect Postgres --environment production --no-ssh`; live at `kotncup.com`, confirmed receiving real test orders.
- [x] Asset pipeline lesson (recurred 3+ times across new patch drops): always `im.crop(im.getbbox())` **before** resizing — several client-supplied source PNGs had huge transparent margins (content as low as 4-15% of canvas), causing tiny/broken renders if cropped naively. CHAIN patch needed `grid-row: span 2` instead of further cropping — its art is genuinely tall/narrow (~0.4:1), not a bad crop.
- [x] Latest commit: `3956458` "Preload stencils and patches without needing a click, via fetchpriority" — replaced click-gated deferred stencil loading with `fetchpriority="low"` eager loading on all 36 stencils + 8 patches from page load.
- [ ] **OPEN REGRESSION** — see [KNOWN_ISSUES.md](KNOWN_ISSUES.md) "Nike Studio Fleece image loading regression after fetchpriority change". Explicitly deferred by user request ("dont fix this now") to protect event-prep time; top priority for the next session.
- [ ] No automated tests were written for the Nike flow this session — everything was verified manually via browser walkthrough on both themes. Existing 276-test suite is green (unaffected — pure new-slug-branch addition) but provides zero coverage of `_post_nike_studio_fleece()` validation, order-numbering, or the dashboard card. Worth a `test_nike_studio_fleece.py` pass modeled on `customer/tests/test_kotn_popup.py` once the event-week churn settles.
- [ ] Stitched-lightning patch swap (`light.png` → `patch-lightning.png`) was made per user request but never got an explicit "yes that matches" confirmation — verify visually next session.
