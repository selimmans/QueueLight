...
- [x] Commit `3956458` "Preload stencils and patches without needing a click, via fetchpriority" — replaced click-gated deferred stencil loading with `fetchpriority="low"` eager loading on all 36 stencils + 8 patches from page load. **Regressed load times for hoodies and patches** (user report post-deploy) — reverted clean in `51267a5` back to the `95fdea2` state (preload-on-hoodie-select for just the 2 relevant pools, patches back to `loading="lazy"`). See [KNOWN_ISSUES.md](KNOWN_ISSUES.md), now RESOLVED.
